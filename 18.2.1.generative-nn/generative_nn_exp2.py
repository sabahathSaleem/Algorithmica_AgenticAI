...
class GenerativeNetwork(nn.Module):
    def __init__(self, vocab_size, emb_dim, hidden_dim, max_len):
        ...
        self.attn_layer = nn.MultiheadAttention(embed_dim=emb_dim, num_heads=1, batch_first=True)
    ...
    def forward(self, x):
        ...
        x,_ = self.attn_layer(x, x, x, attn_mask=mask)
...
def inference(model, tokenizer, start_phrase, context_length, max_gen_length=5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.eval() 
...
if __name__ == "__main__":
    ...
    model = GenerativeNetwork(tokenizer.vocab_size, emb_dim=32, hidden_dim=64, max_len=CONTEXT_SIZE)
    print(model)
    summary(model)
...
# Generated Text: we write clean python code to process training datasets and compute loss parameters . building training
# i love deep learning and
# Generated Text: i love deep learning and i love building neural networks from scratch . neural networks
# deep neural networks
# Generated Text: deep neural networks use layers of linear math and activation functions to learn