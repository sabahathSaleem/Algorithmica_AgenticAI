...
class CustomDataset(Dataset):
    def __init__(self, file_path):
        ...
        X = np.array(self.df[["x1", "x2", "x3"]], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    train_dataset = CustomDataset(base_dir / "non-linear2/train.csv")
    val_dataset = CustomDataset(base_dir / "non-linear2/test.csv")

    model = RegressionNetwork(3, 5, 1)
    batch_size = 10
    epochs = 100

    train(model, epochs, batch_size, train_dataset, val_dataset)
    print(model.state_dict())
    torch.save(model, base_dir / "models/model3.pth")